...
export const WidgetConfigScreen: React.FC<WidgetConfigScreenProps> = ({
  ...
  const configureNoteWidget = async (node: Node) => {
    try {
      ...
      console.log('🔍 [WidgetConfig] Configuring note widget for node:', node.text, 'ID:', node.id);

      // Get fingerprint from storage
      const fingerprint = await AsyncStorage.getItem('@locus_user_fingerprint');
      ...
      console.log('🔍 [WidgetConfig] Generating widget token for note widget...');
      ...
      console.log('🔍 [WidgetConfig] Cleared old note widget data');

      // Save node with widget token and fingerprint
      await WidgetService.updateNoteWidget(node.id, node.text, widgetToken, fingerprint);

      console.log('✅ [WidgetConfig] Note widget configured successfully');

      // Verify what was saved
      const savedData = await WidgetService.getNoteWidgetData();
      console.log('🔍 [WidgetConfig] Saved note widget data:', JSON.stringify(savedData, null, 2));

      // Load updated widget data first
      await loadCurrentWidgetData();

      // Then show success message without closing the widget type selector
      Alert.alert(
        'Success',
        `Note widget configured for "${node.text}". The widget will automatically fetch the latest note content.`,
  ...
  const configureChildWidget = async (node: Node) => {
    try {
      ...
      // Save parent node with widget token and fingerprint
      await WidgetService.updateChildWidget(node.id, node.text, widgetToken, fingerprint);
        ...
        <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
          ...
          <View style={styles.userSection}>
            ...
            <View style={styles.userCard}>
              <Icon name="user-circle" size={24} color="#6B7280" />
