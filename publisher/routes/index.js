...
router.get('/', async (req, res, next) => {
	const user = await getUserBySlug({
		slug: res.locals.domainSlug, customDomain: res.locals.customDomain,
	})
	...
	const notes = JSON.parse(JSON.stringify(await getAllNotes({
		slug: res.locals.domainSlug, customDomain: res.locals.customDomain,
	})))
			...
			if (match) {
				try {
					...
					content = content.map((x) => {
						...
						return {
							lines: Math.ceil(cleanedupText.length / 60), content: cleanedupText,
						}
					})
					...
					for (let i = 0; i < content.length; i++) {
						if (lineCount + content[i].lines > 3) {
							...
							const truncatedContent = content[i].content.substring(0, charsLeft)
	...
	res.render('index', {
		notes: Object.keys(notesByYear).sort((a, b) => Number(b) - Number(a)).map((year) => {
			return {
				yr: year, notes: notesByYear[year],
			}
		}), mainPage: true, ...getCommonDeets(req, res, '/', user),
	})
})
...
router.get('/about', async (req, res, next) => {
	...
	res.render('about', {
		aboutPage: true, notes, bio: user.bio, pic: user.pic, ...getCommonDeets(req, res, '/about', user, {
			title: `About`, meta_title: `About`,
		}),
	})
})
...
router.get('/:slug', async (req, res, next) => {
	const note = await getNoteBySlug({
		slug: res.locals.domainSlug, customDomain: res.locals.customDomain, noteSlug: req.params.slug,
	})
	...
	res.render('post', {
		...getCommonDeets(req, res, `/${note.slug}`, user, {
			title: note.title, meta_title: note.title, meta_image, meta_description,
		}), note, published_at: new Date(note.published_at).getTime(),
	})
})
